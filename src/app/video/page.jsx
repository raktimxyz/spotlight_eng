...
const Page = async () => {
    ...
    return (
        <div className="container">
            {data && data.map((prow, pi) =>
                <div key={pi}>
                    <div className="section-area mb-4">
                        <Link className="section-title fs-4" href={`/video/${prow.category.cat_slug}`}>{prow.category.cat_name} <i className="bi bi-arrow-right-short"></i></Link>
                    </div>
                    <div className="row">
                        {prow.video && prow.video.map((row, i) =>
